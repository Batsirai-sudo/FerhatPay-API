...
router.post("/batsiraiferhatpay/checkConfirmation", isAuth, (req, res, next) => {
  let userid = req.session.passport.user;
  console.log("/checkConfirmation");

  let sql =
    "SELECT TopUpTransaction FROM AgenttransactionsChecker  WHERE id = ?";
  ...
  let query = dbConnect.query(sql, [req.body.notifyID], (err, result) => {
    ...
    if (result[0].TopUpTransaction === 2) {
    ...
    if (result[0].TopUpTransaction === 1) {
      ...
      res.send({
        data: result[0].TopUpTransaction,
        authorization: true,
        error: false,
      });
    }
